...
void testmalla(NMalla **cab, int *inix, int *iniy,void *kemon){
    ...
    if( kbhit() ){
        ...
        t=getch();
        if(t==77 && (*cab)->derecha && (*cab)->derecha->band<=2){ /*Movimiento hacia derecha*/
            *inix+=anchonodo;
            (*cab)=(*cab)->derecha;
            setfillstyle(1,BLACK);
            bar(cx,cy,cx+40,cy+44);
        }
        if(t==75 && (*cab)->izquierda && (*cab)->izquierda->band<=2){  /*Movimiento hacia Izquierda*/
        ...
        if(t==80 && (*cab)->abajo && (*cab)->abajo->band<=2){ /*Movimiento hacia Abajo*/
        ...
        if(t==72 && (*cab)->arriba && (*cab)->arriba->band<=2){ /*Movimiento hacia Arriba*/
